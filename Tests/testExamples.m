...
function testExample4(~)
    ...
    % Check solution
    prob.calcBeams(false);
    assert(prob.nIter == 288,'Wrong number of iterations')
    percentUnder = sum(prob.structs{1}.A*prob.x < 81)/prob.structs{1}.nVoxels;
    assert(abs(percentUnder - 0.5589) < 1e-4,'Incorrect initialization')
    ...
    assert(abs(percentOver - 0.6135) < 1e-4,'Incorrect solution')
    percentOver = sum(prob.structs{2}.A*prob.x > 50)/prob.structs{2}.nVoxels;
    assert(abs(percentOver - 0.4527) < 1e-4,'Incorrect solution')
    percentOver = sum(prob.structs{2}.A*prob.x > 60)/prob.structs{2}.nVoxels;
    assert(abs(percentOver - 0.2324) < 1e-4,'Incorrect solution')
end
...
function testExample5(~)
    ...
    % Check solution
    prob.calcBeams(false);
    assert(prob.nIter == 75,'Wrong number of iterations')
    percentOver = sum(prob.structs{3}.A*prob.x > 50)/prob.structs{3}.nVoxels;
    assert(abs(percentOver - 0.5691) < 1e-4,'Incorrect solution')
    percentOver = sum(prob.structs{4}.A*prob.x > 30)/prob.structs{4}.nVoxels;
    assert(abs(percentOver - 0.3741) < 1e-4,'Incorrect solution')
end